...
import type { ProjectTask } from '@/config/projectMapping'
import './index.css'
...
function resolveIcon(name: string, fallback: React.ReactNode) {
  const Icon = ICON_MAP[name]
  return Icon ? <Icon size={20} color="rgb(210, 170, 204)" /> : fallback
}
...
const IndexPage = () => {
  ...
  const tasks: TaskItem[] = useMemo(
    () =>
      config.tasks.map((t: ProjectTask, i: number) => ({
        id: String(i),
        title: t.title,
        desc: t.desc,
        completed: completedSet.has(String(i)),
        icon: resolveIcon(t.icon, <Droplets size={20} color={PRIMARY_PURPLE} />),
      })),
  ...
  return (
    <View className="min-h-screen bg-background">
      ...
      <View
        ...
        style={{ background: 'linear-gradient(135deg, rgb(78, 18, 98) 0%, rgb(138, 118, 142) 100%)' }}
      >
        {/* 项目选择器 + 标题行 */}
        ...
        <Text className="block text-white text-2xl font-semibold">
          术后的第{' '}
          <Text className="font-bold" style={{ fontSize: '28px' }}>
            {daysAfterProcedure}
          </Text>{' '}
          天
        </Text>
        ...
        >
          <View className="flex items-center justify-between mb-2">
            <Text className="block text-white text-xs opacity-90">今日进度</Text>
        ...
        <Card className="luxury-shadow border-0 rounded-2xl">
          <CardContent className="p-5">
            <View className="flex items-center justify-between mb-4">
              ...
              >
                {completedCount === tasks.length && tasks.length > 0 ? '已完成' : '进行中'}
              </Badge>
            </View>
              ...
              {tasks.map((task, index) => (
                <View key={task.id}>
                  <View
                    ...
                    onClick={() => {
                      if (task.id === '0' && config.tasks[0]?.icon === 'camera') {
                        if (!task.completed) {
                          toggleTask(task.id)
                        }
                        goToCamera()
        ...
        <View className="flex gap-4">
          ...
          >
            <CardContent className="p-5 flex flex-col items-center gap-3">
              <View
                className="w-12 h-12 rounded-full flex items-center justify-center"
                style={{ backgroundColor: 'rgba(74, 59, 78, 0.08)' }}
              >
